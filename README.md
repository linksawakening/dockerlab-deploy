...
2. The docker-git-deploy skill/tooling in the `docker-git-deploy/` directory.

The top-level files are a working deployment configuration that the framework installs. The framework itself lives in `docker-git-deploy/`.
...
The agent skill is in `docker-git-deploy/templates/docker-git-deploy-skill/SKILL.md`. Install it with:
